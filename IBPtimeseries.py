...
if __name__=='__main__':
	...
	for filename in all_files:
		...
		# Define X dataset
		X_data=data.drop(labels=['Date','Time','IBPS','IBPD','ICU_Stay','Hospital_Stay','HR(BPM)','Outcome'],axis=1)
	...
	#ANN
	ann = Sequential()
	ann.add(Dense(30, activation = 'relu', kernel_initializer='normal',input_shape=(30,)))
	...
	history_ann=ann.fit(X_train, y_train, epochs = 30, verbose=1,validation_split=0.2,batch_size=50,callbacks=[early_stopping])
	...
	rnn = Sequential()
	rnn.add(SimpleRNN(30, input_shape = (30,1 ),activation='relu'))
	...
	history_rnn = rnn.fit(trainX, y_train, epochs = 30, verbose = 1,validation_split=0.2,batch_size=50,callbacks=[early_stopping])
	...
	lstm = Sequential()
	lstm.add(LSTM(input_shape = (30,1),units= 30,  activation='relu',return_sequences = True))
	...
	history_lstm = lstm.fit(trainX, y_train, epochs = 30, verbose=1,validation_split=0.2,batch_size=50, callbacks=[early_stopping])
	
	lstm.save("lstmIBP.h5")
	print("Saved model to disk")
	...
	bilstm = Sequential()
	bilstm.add(Bidirectional(LSTM(units = 30,return_sequences=True),input_shape=(30,1)))
	
	bilstm.add(Bidirectional(LSTM(units = 30)))
	bilstm.add(Dense(30, activation='relu'))
	...
	history_bilstm = bilstm.fit(trainX, y_train, epochs = 30, verbose=1,validation_split=0.2,batch_size=50,callbacks=[early_stopping])
	bilstm.save("bilstmIBP.h5")
	print("Saved model to disk")
